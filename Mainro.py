...
PATH = " "
WENTPATH = "~"
# format (row, startColumn, endColumn + 1)
PATH_DES_TUPLE = (
    (1, 13, 32), 
...
path_dict_list = []
def convertToListDict(pathTupleTuple):
    for pathTuple in pathTupleTuple:
        convertToDict(pathTuple)

def convertToDict(pathDesTuple):
    pathDesDict = {}
    pathDesDict["row"] = pathDesTuple[0]
    pathDesDict["startColumn"] = pathDesTuple[1]
    # The end column number as added 1 beacause it make it easier to calculate
    pathDesDict["endColumn"] = pathDesTuple[2]

    return pathDesDict

maze = Maze(WALL, PATH, WENTPATH, PATH_DES_TUPLE, START, END)
graph = Graph(maze)
