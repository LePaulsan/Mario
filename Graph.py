...
class Graph:
    def __init__(self, maze):
        self.maze = maze
        self.nodeList = self.create_node_list()
        ...
        for i in range(self.maze.height):
            for j in range(self.maze.width):
                if self.maze.is_path(i, j):
                    neighboor = self.maze.get_neighboor_dic(i, j)
                    is_a_conner = (neighboor["left"] or neighboor["right"]) and (neighboor["front"] or neighboor["back"])
                    if is_a_conner:
        ...
        for key in tempDy:
            if len(tempDy[key]) > 1:
                for i in range(len(tempDy[key]) - 1):
                    ...
                    if self.maze.get_neighboor_dic(key, tempDy[key][i])["right"]:
            ...
            if len(tempDx[key]) > 1:
                for i in range(len(tempDx[key]) - 1):
                    ...
                    # if (tempDx[key][i], key) == (10, 14):
                        ...
                        # print("i went here")
                    if self.maze.get_neighboor_dic(tempDx[key][i], key)["back"]:
                        edges.append(nodeSet)
                        # if (tempDx[key][i], key) == (10, 14):
                            # print(self.maze.getNeighboorDic(tempDx[key][i], key)["right"])
                            # print("i went here")
